...
import com.library.ihttp.load.download.DownloadInterceptor;
import com.library.ihttp.load.download.DownloadObserver;
import com.library.ihttp.model.Download;
import com.library.ihttp.retrofit.RetrofitUtils;
import com.library.ihttp.utils.ComputeUtils;
import com.library.ihttp.utils.DBHelper;
import com.library.ihttp.utils.LogUtils;
import com.library.ihttp.utils.RequestUtils;
import com.library.ihttp.utils.ResponseUtils;
...
public class IDownLoad {
    /*单例模式*/
    private volatile static IDownLoad instance;
    /*下载集合*/
    ...
    public static IDownLoad get() {
        if (instance == null) {
            synchronized (IDownLoad.class) {
                if (instance == null) {
                    instance = new IDownLoad();
                }
            }
        }
        return instance;
    ...
    public void startDownload(final Download download) {
        ...
        LogUtils.d("IHttp startDownload:" + download.getServerUrl());
        /*判断本地文件是否存在*/
        ...
        Map<String, Object> headerMap = IHttp.Configure.get().getBaseHeader();
    ...
    public void stopDownload(Download download) {
        ...
        if (download == null) return;
        LogUtils.d("IHttp stopDownload:" + download.getServerUrl());
        /**
         * 1.暂停网络数据
         * 2.设置数据状态
         * 3.更新数据库
    ...
    public void removeDownload(Download download, boolean removeFile) {
        ...
        if (download == null) return;
        LogUtils.d("IHttp removeDownload:" + download.getServerUrl());
        //未完成下载时,暂停再移除
    ...
    public void stopAllDownload() {
        ...
        callbackMap.clear();
        downloadSet.clear();
        LogUtils.d("IHttp stopAllDownload");
    }
