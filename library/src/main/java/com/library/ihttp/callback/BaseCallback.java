package com.library.ihttp.callback;

import com.library.ihttp.IHttp;
import com.library.ihttp.exception.ApiException;
import com.library.ihttp.exception.ExceptionEngine;
import com.library.ihttp.observer.HttpObserver;
import com.library.ihttp.utils.ThreadUtils;
...
public abstract class BaseCallback<T> extends HttpObserver<T> {
    ...
    private void onCanceledLogic() {
        if (!ThreadUtils.isMainThread()) {
            IHttp.Configure.get().getHandler().post(new Runnable() {
                @Override
                public void run() {
                    inCancel();
                }
