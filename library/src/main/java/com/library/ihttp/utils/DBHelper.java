...
//    private static LiteOrm db;
    ...
    private DBHelper() {
        if (IHttp.Configure.get().getContext() == null) {
            throw new NullPointerException("IHttp not init!");
        }
        context = IHttp.Configure.get().getContext();
        initDB(context);
    }
...
//            config.debugged = IHttp.Configure.get().isShowLog(); // open the log
